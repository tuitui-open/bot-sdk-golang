...
测试会发送真实消息、发布团队帖子、读取文件空间、创建并删除临时目录，并全量覆盖 Bot 1 的快捷
指令。快捷指令测试会设置并读取带 `golang` tag 的指令，用于明确标识该指令由 Go SDK 写入。
双 Bot 测试会为 Bot 1 建立 WebSocket；运行前应停止其他使用相同凭据接收事件的进程。

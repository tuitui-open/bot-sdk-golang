...
func (e *EventAPI) GetMessageMedia(data any) []MessageMedia { return GetMessageMedia(data) }
func (e *EventAPI) RenderMessageBody(data any) string       { return RenderMessageBody(data) }
...
func (s *Subscription) normalizeMessage(event RawEvent) error {
	...
	if ref, ok := data["ref"].(map[string]any); ok {
...
func (s *Subscription) normalizeSharedPost(message map[string]any) error {
...
func GetMessageMedia(data any) []MessageMedia {
	value, ok := data.(map[string]any)
	if !ok {
		return nil
	}
	result := []MessageMedia{}
	appendURL := func(raw any, mime string) {
...
func RenderMessageBody(data any) string {
	value, ok := data.(map[string]any)
	if !ok {
		return ""
	}
	parts := renderBaseMessage(value)
	if ref, ok := value["ref"].(map[string]any); ok {
